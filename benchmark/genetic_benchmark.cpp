...
// Remove the insertionSortPrograms function and replace with a linear search
std::pair<genetic::program*, genetic::program*> getTop2Programs(genetic::program* programs, int size) {
...
void run_symbolic_regression(const std::string &dataset_file) {
  ...
  auto [best1, best2] = getTop2Programs(final_programs, params.population_size);

  std::vector<float> y_pred1(X_test.size());
  genetic::symRegPredict(X_test_flat.data(), X_test.size(), best1,
                         y_pred1.data());

  std::vector<float> y_pred2(X_test.size());
  genetic::symRegPredict(X_test_flat.data(), X_test.size(), best2,
                         y_pred2.data());
